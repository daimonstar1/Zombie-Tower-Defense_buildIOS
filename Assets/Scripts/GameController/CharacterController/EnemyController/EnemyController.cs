...
//    spriteRenderer = Master.GetChildByName (gameObject, "Sprite").GetComponent<SpriteRenderer> ();
  ...
  {
    if (stopAllAction || status.IsFreezing || status.IsAttacking)
      return;
  ...
  {
    if (stopAllAction || status.IsFreezing)
      return;
    ...
    if (status.IsCanAttack) {
      if (!isFirstAttack) {
        ...
        timeToAttack -= Time.deltaTime * 15;
        if (timeToAttack <= 0) {
          Attack ();
        }
        //if (!isWaitForAttack)
...
//    spriteRenderer.sortingOrder = -1000 - (int)gameObject.transform.localPosition.y + 10;
    if (isSpineAnim) {
      if (anim == null)
        anim = transform.GetComponent<SkeletonAnimation> ();
      anim.GetComponent<MeshRenderer> ().sortingOrder = -1000 - (int)gameObject.transform.localPosition.y + 10;
    } 
  }
  ...
  private void playAnimSpine (string animationClipName, System.Action onAnimationComplete = null)
  {
    ...
    bool loop = false;
    if (animationClipName.Contains ("Dead"))
      loop = false;
    else
      loop = true;
    if (isSpineAnim) {
    ...
    anim.loop = loop;
    float duration = 0;
    if (anim.skeletonDataAsset.GetSkeletonData (true).FindAnimation (animationClipName) != null)
      duration = anim.skeletonDataAsset.GetSkeletonData (true).FindAnimation (animationClipName).duration;
    Master.WaitAndDo (0.01f, () => {
      StartCoroutine (DetectAnimationOnComplete_Spine (onAnimationComplete, duration));
    }, this);
  }

  IEnumerator DetectAnimationOnComplete_Spine (System.Action onComplete = null, float duration = 0)
  {
