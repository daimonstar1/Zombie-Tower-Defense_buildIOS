...
public class EnemyDataController : MonoBehaviour
{
	...
	public class IncreaseStatsPerLevel
	{
		...
		public static float health = 10.5f;
		public static float moveSpeed = 0f;
		public static float attackSpeed = 0f;
	}

	// private float increasePercentPerLevel = 25; //%
	...
	public EnemyData GetEnemyDataWithUpgradeByID (string id, GameplayController.GameDifficult gameDifficulte = GameplayController.GameDifficult.Normal)
	{
		EnemyData enemyData = GetEnemyDataByID (id);
		if (enemyData != null) {
			EnemyData newEnemyData = new EnemyData ();
			if (gameDifficulte == GameplayController.GameDifficult.Normal) {
				newEnemyData = UpgradeEnemy (enemyData, (Master.LevelData.currentLevel - 1));
			} else {
				newEnemyData = UpgradeEnemy (enemyData, Master.LevelData.totalLevel + Master.LevelData.currentLevel + 2);
			}
			return UpgradeEnemy (newEnemyData, Master.Level.currentWave, 100);
		}
		return null;
	}
	...
	{
		return ObscuredPrefs.GetInt ("EnemyUnlock_" + enemyID, 0) == 0 ? false : true;
	}

	public void UnlockEnemy (string enemyID)
	{
		ObscuredPrefs.SetInt ("EnemyUnlock_" + enemyID, 1);
		ObscuredPrefs.Save ();
	}
	...
	public EnemyData[] GetEnemiesAtLevel (int level)
	{
		List<EnemyData> enemiesData = new List<EnemyData> ();

		foreach (string enemyID in Master.LevelData.GetEnemiesAtLevel(level)) {
			if (GetEnemyDataByID (enemyID) != null) {
				EnemyData temp = GetEnemyDataByID (enemyID);
				if (temp != null)
					enemiesData.Add (temp);
			}
		}

		enemiesData = enemiesData.OrderBy (x => x.EnemyID).ToList ();
		return enemiesData.ToArray ();
	}
