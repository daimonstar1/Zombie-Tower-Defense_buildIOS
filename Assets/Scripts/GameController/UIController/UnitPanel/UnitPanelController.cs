...
public class UnitPanelController : MonoBehaviour
{
  ...
  public class UnitPreview
  {
    ...
    public static Transform unitPosition;
    public static UILabel unitNameLabel;
    public static UILabel unitLevel;
    public static UISlider unitExpSlider;
  ...
  void Update ()
  {
    unitShow.transform.localScale = new Vector3 (55, 55, 55);
  }
  ...
  public void OnOpen ()
  {
    SetListUnit ();
    SetInfo ();
    Master.Tutorial.CheckAndContinueNextStepTutorial (TutorialController.TutorialsIndex.UpgradeStatsOfUnit);
  ...
  void AssignObject ()
  {
    ...
    UnitPreview.unitPosition = Master.GetChildByName (unitPanel, "UnitPos").transform;

    UnitPreview.unitNameLabel = Master.GetChildByName (unitPanel, "UnitNameLabel").GetComponent<UILabel> ();
    UnitPreview.unitLevel = Master.GetChildByName (unitPanel, "UnitLevelLabel").GetComponent<UILabel> ();
    UnitPreview.unitExpSlider = Master.GetChildByName (unitPanel, "UnitExpSlider").GetComponent<UISlider> ();
  ...
  void SetListUnit ()
  {
    ...
    foreach (UnitDataController.UnitData unitData in Master.UnitData.listUnitData) {
      ...
      UI2DSprite icon = Master.GetChildByName (unit, "Icon").GetComponent<UI2DSprite> ();
      ...
      if (unitData.UnlockAtLevel <= (Master.LevelData.lastLevel + 1)) {
        icon.sprite2D = Resources.Load<Sprite> ("Textures/Characters/Units/Unit_" + unitData.UnitID + "/Icon");
        ...
        icon.sprite2D = Resources.Load<Sprite> ("Textures/Characters/Units/unit_lock");
  ...
  private void SetProgressUpgrade (int value, List<UITexture> listProgressTextures)
  {
    ...
    foreach (UITexture item in listProgressTextures) {
      item.mainTexture = Resources.Load<Texture2D> ("Textures/UI/upgrade_progress_icon_disable1");
    }

    for (int i = 0; i < value; i++) {
      listProgressTextures [i].mainTexture = Resources.Load<Texture2D> ("Textures/UI/upgrade_progress_icon_enable1");
    }
  }
  ...
  public void SetUnitSelectButtonTexture ()
  {
    foreach (GameObject item in listUnitSelectButton) {
      if (item.name.Split ('_') [1] == unitSelectedID) {
        item.GetComponentInChildren<UI2DSprite> ().color = new Color (1, 1, 1, 1);
      } else {
        item.GetComponentInChildren<UI2DSprite> ().color = new Color (146 / 255f, 146 / 255f, 146 / 255f, 1f);
      }
    }
  }
