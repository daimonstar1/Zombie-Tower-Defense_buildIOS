...
public class FreeRewardController : MonoBehaviour
{
	...
	public static int TimeRemainingWatchVideo_Offerwall ()
	{
		string dateTimeGetRewarStr = PlayerPrefs.GetString ("DateTimeGetRewardWatchVideo_Offerwall", "");
	...
	public static void SetDateTimeGetRewardWatchVideo_Offerwall ()
	{
		DateTime currentDateTime = DateTime.Now.AddMinutes (minuteGetRewardWatchVideo * 2);
		string dateTimeStr = currentDateTime.ToString ("yyyy-MM-dd HH:mm:ss");
		PlayerPrefs.SetString ("DateTimeGetRewardWatchVideo_Offerwall", dateTimeStr);
		PlayerPrefs.SetString ("CurrentWatchVideoReward_Offerwall", "");
		PlayerPrefs.Save ();
	}
